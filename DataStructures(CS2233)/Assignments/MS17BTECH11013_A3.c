...
void postOrderTraversal(struct bstNode *node );
...
int nofChildren(struct bstNode* root);
void printReplacement(struct bstNode* root,struct bstNode* node, int count);
...
void printReplacement(struct bstNode* root,struct bstNode* node, int count)
{
     if(count > 0)
     {
         if(count == 1) // if only one child
         {
             if(node->left != NULL) // if left child is not null then print left
             {
                 printf("%s\n", node->left->data);
             }
             else   //  left is null so 1 child has to be right print it.
             {
                 printf("%s\n", node->right->data);
             }
         }
         else  // count is 2 so find it's sucessor
         {
             struct bstNode *suc = Sucessor(node);
